...
styles = ["Top Load", "Front Load", "Side-by-Side", "Compact", "Standard"]
colors = ["White", "Black", "Stainless Steel", "Gray", "Beige"]
conditions = ["NEW", "USED"]
...
def seed_machines(n=20):
    ...
    type_ids = [t.id for t in Types.query.all()]
    if not type_ids:
    ...
    for i in range(n):
        machine = Machine(
            ...
            finished_on=datetime.date.today() - datetime.timedelta(days=random.randint(0, 30)),
