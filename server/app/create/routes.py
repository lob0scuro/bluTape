...
def create_repair():
    try:
        ...
        newMachine = Machine(brand=brand, model=model.upper(), serial=serial.upper(), color=color, style=style, vendor=vendor, condition=condition.upper(), heat_type=heat_type if heat_type else None, machine_type=machine_type)
